...
function SignupForm() {
  ...
  const passwordError = passwordTouched && !isPasswordValid ? "Password must be at least 6 characters." : null;
  const confirmError = confirmTouched && !isConfirmValid ? "Passwords do not match." : null;
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if backend returned session token
      if (data?.access_token && data?.id) {
        // Update AuthContext state and localStorage synchronously
        setSession(data.access_token, data.id, data.email);
        
        // Confirm state application and navigate directly to dashboard
        router.push("/dashboard");
      } else {
        throw new Error("Account created, but authentication token was missing. Please try logging in.");
      }
    } catch (err: any) {
      ...
      <form className="space-y-4" onSubmit={handleSignup} noValidate>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-11 pr-4 py-2.5 rounded-xl glass-input text-sm outline-none transition-all ${
                  emailError ? "border-rose-500/80 focus:border-rose-500" : ""
                }`}
            ...
            {emailError && (
              <p className="text-[11px] text-rose-400 mt-1 flex items-center gap-1">
                <AlertCircle className="w-3 h-3 inline" /> {emailError}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-11 pr-11 py-2.5 rounded-xl glass-input text-sm outline-none transition-all ${
                  passwordError ? "border-rose-500/80 focus:border-rose-500" : ""
                }`}
            ...
            {passwordError ? (
              <p className="text-[11px] text-rose-400 mt-1 flex items-center gap-1">
                <AlertCircle className="w-3 h-3 inline" /> {passwordError}
              </p>
            ) : (
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                required
                className={`w-full pl-11 pr-11 py-2.5 rounded-xl glass-input text-sm outline-none transition-all ${
                  confirmError ? "border-rose-500/80 focus:border-rose-500" : ""
                }`}
            ...
            {confirmError && (
              <p className="text-[11px] text-rose-400 mt-1 flex items-center gap-1">
                <AlertCircle className="w-3 h-3 inline" /> {confirmError}
              </p>
            )}
            {!confirmError && isConfirmValid && (
        ...
        <button
          ...
          className={`w-full flex justify-center items-center gap-2 py-3 px-4 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 shadow-lg shadow-indigo-600/30 transition-all ${
            loading || (emailTouched && !isFormValid) ? 'opacity-70 cursor-not-allowed' : ''
          }`}
        >
          {loading ? (
